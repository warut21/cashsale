...
begin
    ...
    strline := tab1+'เวลา : '+FormatDateTime('hh:nn:ss ',now)+Thaimid('รายงานสรุปการขาย ประจำวันที่ : '+Formatdatetime('DD/MM/YYYY',StrToDate(Edit1.Text))+
                ' ประจำเครื่อง : '+ThaiLeft(Label6.Caption,3),66);
    MemoReport.Lines.Add(strline);
    strline := tab1+tab1+tab1+Thaimid('แผนกสินค้า : '+ThaiRight(Group_id.Text,3),66);
    MemoReport.Lines.Add(strline);
    MemoReport.Lines.Add(lines);
    strline := tab2+Thaimid('รหัสสินค้า       ',20)+ThaiRight('ราคาขาย',10)+ThaiRight('ปริมาณขายสะสม',15)+ThaiRight('ยอดขายสะสม',15)+ThaiRight('ปริมาณขายวันนี้',15)+ThaiRight('ยอดขายวันนี้',15);
    MemoReport.Lines.Add(strline);
    MemoReport.Lines.Add(lines);
    MemoReport.Lines.Add(' ');
end;
...
begin
      ...
      sqls := 'SELECT BILL_HEAD.BILLH_COMP_ID, '+
              ...
              'AND BILL_HEAD.BILLH_COMP_ID = '''+FrmBill.EdtComp_id.Text+''' AND BILL_DETAIL.BILLD_STATUS = ''A'' AND FINISH_GOODS.FG_GRP = '''+Group_id.Text+''' AND BILL_HEAD.BILLH_DATE <= DateValue('''+date+''') '+
      ...
      if FrmDatamodule.ADOQuery1.RecordCount <> 0 then
        ...
        begin
          ...
          strline := tab2+thaileft(fieldvalues['BILLD_GOODS_ID'],20)+
                     ...
                     thairight(formatcurr('#,##0.00',fieldvalues['SumNum']),15)+
                     thairight(formatcurr('#,##0.00',fieldvalues['SumAmt']),15);
          ...
          if (FrmDatamodule.ADOQuery2.FieldValues['BILLD_GOODS_ID'] = fieldvalues['BILLD_GOODS_ID'])
            ...
            begin
            strline := strline+thairight(formatcurr('#,##0.00',FrmDatamodule.ADOQuery2.fieldvalues['SumNum']),15)+
                       thairight(formatcurr('#,##0.00',FrmDatamodule.ADOQuery2.fieldvalues['SumAmt']),15);
            ...
            begin
            strline := strline+thairight(formatcurr('#,##0.00',0),15)+
        ...
        MemoReport.Lines.Add(lines);
        strline := tab2+Thaimid('รวมรายการทั้งหมด :  ',20)+ThaiRight(IntToStr(wline-1),8)+thairight(formatcurr('#,##0.00',sum1),17)+
                  thairight(formatcurr('#,##0.00',sum2),15)+thairight(formatcurr('#,##0.00',sum3),15)+thairight(formatcurr('#,##0.00',sum4),15);
        MemoReport.Lines.Add(strline);
        MemoReport.Lines.Add(lines);
        end//end then
      else
        begin
        strline := tab2+thaimid('ไม่มีข้อมูลรายการขายในแผนกสินค้านี้',90);
        MemoReport.Lines.Add(strline);
        end;//end else
    end; //end with
end;
...
procedure TFrmReport1.Edit1Change(Sender: TObject);
begin
    MemoReport.Clear;
end;
