  ...
  'A':begin
        ...
        if chk = True then begin
          ...
          if (itmdata[1] <> '')and (itmdata[2] <> '')and (itmdata[3] <> '')and (itmdata[4] <> '')and (itmdata[5] <> '')and (itmdata[6] <> '')and (itmdata[7] <> '') then begin
            ...
            with ADOCmdCompany do begin
              CommandText := 'INSERT INTO COMPANY (COMP_ID, COMP_NAME, COMP_ADD1, COMP_ADD2, COMP_ADD3, COMP_POST, COMP_TAX)'+
              'VALUES (:newid, :newname, :newadd1, :newadd2, :newadd3, :newpost, :newtaxid)';
              CommandType := cmdText;
