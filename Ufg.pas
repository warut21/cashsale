  ...
  case state of
    'F': begin
        ...
        if numallrec <> 0 then begin
          ...
          EdShort.Text := ADODSFG.FieldValues['FG_SHORT'];
          EdBarcode.Text := ADODSFG.FieldValues['FG_BAR_CODE'];
          EdGrade.Text := ADODSFG.FieldValues['FG_GRADE'];
          EdDes.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_DES']);
    ...
    'A': begin
        try // Check number input
          itmdata[9] := FormatFloat('#0.00', StrToFloat(Trim(EdPrice.Text)));
          itmdata[10] := FormatFloat('#0.00', StrToFloat(Trim(EdCost.Text)));
          if (StrToFloat(EdPrice.Text) >= 0) and (StrToFloat(EdCost.Text) >= 0) then
           begin
            //Get data from input screen
            itmdata[1] := Uppercase(Trim(EdFGId.Text));
            itmdata[2] := CbGrpId.Text;
            itmdata[3] := CbUnitsId.Text;
            itmdata[4] := Uppercase(Trim(EdShort.Text));
            itmdata[5] := Uppercase(Trim(EdBarcode.Text));
            itmdata[6] := Uppercase(Trim(EdGrade.Text));
            itmdata[7] := EdDes.Text;
            itmdata[8] := EdWh.Text;
            // Check primary key
            ...
            if ADODSFG.RecNo <= 0 then
             ...
             // Check Barcode iterate
             with ADOCmdFG do begin
              CommandText := 'SELECT * FROM FINISH_GOODS ' +
                'WHERE FG_BAR_CODE = :barcode ';
              CommandType := cmdText;
              Parameters.ParamByName('barcode').Value := itmdata[5];
              ADODSFG.Recordset := Execute;
              end;
           ...
           // Check FG SHORT
            with ADOCmdFG do begin
              CommandText := 'SELECT * FROM FINISH_GOODS ' +
                'WHERE FG_SHORT = :fgshort ';
              CommandType := cmdText;
              Parameters.ParamByName('fgshort').Value := itmdata[4];
              ADODSFG.Recordset := Execute;
            end;
            ADODSFG.First;
            if ADODSFG.RecNo <= 0  then
            ...
            if chk1 = True then
              begin
                StabrFG.Panels[1].Text := 'ไม่สามารถบันทึกข้อมูลได้เพราะป้อนข้อมูลรหัสสินค้าซ้ำ';
              ...
              begin
                if chk2 = True then
                  begin
                    StabrFG.Panels[1].Text := 'ไม่สามารถบันทึกข้อมูลได้เพราะป้อนข้อมูลรหัสบาร์โค้ดซ้ำ';
                    StabrFG.Panels[2].Text := ' คำแนะนำ : กรุณากดปุ่มCancel แล้วบันทึกข้อมูลใหม่อีกครั้ง';
                  end
                else
                  begin
                    if chk3 = True then
                      begin
                        StabrFG.Panels[1].Text := 'ไม่สามารถบันทึกข้อมูลได้เพราะป้อนข้อมูลรหัสย่อสินค้าซ้ำ';
                        StabrFG.Panels[2].Text := ' คำแนะนำ : กรุณากดปุ่มCancel แล้วบันทึกข้อมูลใหม่อีกครั้ง';
                      end
                    else
                      begin
                      // Check not null
                      if (itmdata[1] <> '') and (itmdata[2] <> '') and (itmdata[3] <> '') and (itmdata[4] <> '') and (itmdata[5] <> '') and (itmdata[6] <> '') and (itmdata[7] <> '') and (itmdata[8] <> '') and (itmdata[9] <> '') then
                      begin
                        ...
                        with ADOCmdFG do
                        begin
                          CommandText := 'INSERT INTO FINISH_GOODS (FG_ID, FG_SHORT, FG_GRP, FG_UNITS, FG_BAR_CODE, FG_GRADE, FG_DES, FG_WH, FG_PRICE, FG_COST)' +
                            'VALUES (:fgid , :fgshort, :fggrp , :units , :barcode , :grade , :des , :wh , :price , :cost)';
                          CommandType := cmdText;
                          Parameters.ParamByName('fgid').Value := itmdata[1];
                          ...
                          Parameters.ParamByName('fgshort').Value := itmdata[4];
                          Parameters.ParamByName('barcode').Value := itmdata[5];
                          Parameters.ParamByName('grade').Value := itmdata[6];
                          Parameters.ParamByName('des').Value := itmdata[7];
                          Parameters.ParamByName('wh').Value := itmdata[8];
                          Parameters.ParamByName('price').Value := itmdata[9];
                          Parameters.ParamByName('cost').Value := itmdata[10];
                          Execute
                        end;
                        EdFGId.Text := ''; // Plus
                        CbGrpId.Text := ''; // Plus
    ...
    'E': begin
        ...
        EdCost.Enabled := True;
        if numallrec <> 0 then begin
          EdFGId.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_ID']);
          CbGrpId.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_GRP']);
          CbUnitsId.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_UNITS']);
          EdShort.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_SHORT']);
          EdBarcode.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_BAR_CODE']);
          EdGrade.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_GRADE']);
          EdDes.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_DES']);
          EdWh.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_WH']);
          EdPrice.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_PRICE']);
          EdCost.Text := FrmDataModule.Ck_null(ADODSFG.FieldValues['FG_COST']);
          StabrFG.Panels[1].Text := ' กำลังแสดงข้อมูลของ record ที่ ' + InttoStr(numrec) + ' จากทั้งหมด ' + InttoStr(numallrec) + ' record';
    ...
    'U': begin
        try // Check number input
          itmdata[9] := formatfloat('#0.00', strtofloat(EdPrice.Text));
          itmdata[10] := formatfloat('#0.00', strtofloat(EdCost.Text));
           ...
           // Check FG SHORT
            ...
            begin
              ...
              begin
                ...
                if (itmdata[1] <> '') and (itmdata[2] <> '') and (itmdata[3] <> '') and (itmdata[10] <> '') and (itmdata[5] <> '') and (itmdata[6] <> '') and (itmdata[7] <> '') and (itmdata[8] <> '') and (itmdata[9] <> '') then begin
                with ADOCmdFG do begin
                  CommandText := 'UPDATE FINISH_GOODS ' +
                    'SET ' + //FINISH_GOODS.FG_ID = :fgid,'+
                    'FINISH_GOODS.FG_GRP = :fggrp,' +
                    'FINISH_GOODS.FG_UNITS = :units,' +
                    'FINISH_GOODS.FG_SHORT = :fgshort,' +
